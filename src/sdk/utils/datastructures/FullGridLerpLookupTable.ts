 ...
 */
export class FullGridLerpLookupTable {
   ...
   */
  public constructor(values: readonly number[], ...keys: (readonly number[])[]) {
    let valueCount = 0;
    if (keys.length > 0) {
      valueCount = keys.reduce((outputCount, dimension, index) => {
        if (dimension.length === 0) {
          throw new Error(`FullGridLerpLookupTable: dimension ${index} has zero breakpoint keys`);
        }
    ...
    if (valueCount !== values.length) {
      throw new Error(`FullGridLerpLookupTable: number of values (${values.length}) does not match the expected number of values from the provided breakpoint keys (${valueCount})`);
    }
    ...
    this.values = new Float64Array(valueCount);
    for (let sortedIndex = 0; sortedIndex < valueCount; sortedIndex++) {
      ...
      this.values[sortedIndex] = values[unsortedIndex];
    }
    ...
    this.lookupCache = new Float64Array(2 ** this.dimensionCount);
  }
  ...
  private findKeyIndexBinary(keys: Float64Array, query: number): number {
    return ArrayUtils.binarySearch(keys, query, FullGridLerpLookupTable.keyComparator);
  }
  ...
  public get(...key: number[]): number {
    if (this.dimensionCount === 0) {
      throw new Error('FullGridLerpLookupTable::get(): cannot look up a key in a 0-dimensional table');
    }

    if (key.length < this.dimensionCount) {
      throw new Error(`FullGridLerpLookupTable::get(): cannot look up a ${key.length}-dimensional key in a ${this.dimensionCount}-dimensional table`);
    }
    ...
    for (let i = 0; i < codeCount; i++) {
      ...
      this.lookupCache[i] = this.getBoundsBreakpointValue(this.boundsKeyIndexCache, code);
    }
    ...
    for (let dimension = this.dimensionCount - 1; dimension >= 0; dimension--) {
      ...
      for (let i = 0; i < count; i++) {
        const lower = this.lookupCache[i * step];
        const upper = this.lookupCache[(i + 0.5) * step];
        this.lookupCache[i * step] = lower + (upper - lower) * weight;
      }

      step *= 2;
    }

    return this.lookupCache[0];
  }
   ...
   */
  private getBoundsBreakpointValue(boundsKeyIndexes: Float64Array, boundsCode: Float64Array): number {
    let valueIndex = 0;
    let mod = 1;
