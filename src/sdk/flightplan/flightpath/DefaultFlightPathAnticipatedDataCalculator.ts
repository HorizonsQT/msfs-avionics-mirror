...
export class DefaultFlightPathAnticipatedDataCalculator implements FlightPathAnticipatedDataCalculator {
  ...
  public getAnticipatedData(legs: LegDefinition[], startIndex: number, endIndex: number, dataContext: FlightPathAnticipatedDataContext,
    ...
    if (!this.useActualSpeedsOnly && legs.length > 3 && mapLegIndex >= 0) {
      ...
      let distanceToLandingMt = 0;

      const prevLocation = new GeoPoint(mapLeg.calculated?.endLat ?? 0, mapLeg.calculated?.endLon ?? 0);

      const fafAltMt = finalAltitudeMt + 2000 * Avionics.Utils.FEET2METER; // FAF is simply set as MAP alt + 2000 ft approximately

      // Now we are ready to once more iterate through all flightplan legs:
      for (let calculatedIndex = legs.length - 1; calculatedIndex >= 0; calculatedIndex--) {
        if (calculatedIndex >= mapLegIndex) {
          ...
          // based on that. The distance from landing is not taken from the cumulative LNAV distances but is calculated independently
          // to break the circular dependency which otherwise can create jumping flight paths (especially if the round-the-world
          // flight path is generated by accident):
          const flightplanLeg = legs[calculatedIndex];

          // We only anticipate speeds for the descent, so break from the loop when earlier phases are reached:
          if (flightplanLeg.verticalData.phase !== 'Descent') {
            break;
          }
          ...
          // Use any static waypoint which has a fix and is not calculated to cumulate a distance to landing, which then is used to come up
          // with an anticipated altitude and speed. If this leg does not qualify, simply continue with the latest distanceToLandingMt,
          // which potentially results in an error with the tendency to make turns tighter than needed (which is acceptable):
          if (FlightPlanUtils.isToFixLeg(flightplanLeg.leg.type) && flightplanLeg.calculated?.endLat != undefined && flightplanLeg.calculated?.endLon != undefined) {
            distanceToLandingMt += prevLocation.distance(flightplanLeg.calculated.endLat, flightplanLeg.calculated.endLon);
            prevLocation.set(flightplanLeg.calculated.endLat, flightplanLeg.calculated.endLon);
          }
          ...
          const anticipatedAltitudeMt = distanceToLandingMt * Math.tan(3 * Avionics.Utils.DEG2RAD) + finalAltitudeMt;
          ...
          tasKnots = Math.round(tasKnots); // Round tas to 1 knot

          const distanceToLegMt = flightplanLeg.calculated?.cumulativeDistance;
          if (totalFlightDistanceMt && distanceToLegMt) {
            const currentToLegDistanceMt = currentToDestDistanceMt - (totalFlightDistanceMt - distanceToLegMt);
