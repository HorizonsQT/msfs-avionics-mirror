...
import { WTLineLegacyFmsPosEvents } from '../fms/WTLineFmsTypes';
import { BearingDistPredictions, FlightPlanIntersection, LatLonPredictions, WTLineFixInfoCalculatedData, WTLineFixInfoData } from './WTLineFixInfoData';

/** A calculator for fix info intersections and predictions data */
export class WTLineFixInfoCalculator {
  ...
  constructor(
    ...
    // FIXME don't rely on that enum, it was originally for the WT21
    this.fmsPositionAvailable.setConsumer(this.bus.getSubscriber<WTLineLegacyFmsPosEvents>().on('fms_pos_valid'));
   ...
   */
  public calculateEtaAltitudePrediction(plan: FlightPlan, fixInfo: WTLineFixInfoData, calcData: WTLineFixInfoCalculatedData): void {
      ...
      if (distance <= (leg.calculated.cumulativeDistanceWithTransitions ?? 0)) {
        ...
        for (const vector of leg.calculated.flightPath) {
          ...
          if (desiredDistanceAlongVector <= vector.distance) {
            const vecCircle = FlightPathUtils.setGeoCircleFromVector(vector, WTLineFixInfoCalculator.geoCircleCache[0]);
            const vecStart = WTLineFixInfoCalculator.geoPointCache[0].set(vector.startLat, vector.startLon);
            const point = vecCircle.offsetDistanceAlong(
              vecStart,
              UnitType.GA_RADIAN.convertFrom(desiredDistanceAlongVector, UnitType.METER),
              WTLineFixInfoCalculator.vec3Cache[0],
              MathUtils.HALF_PI,
            );
   ...
   * @param calcData Output data for the CDU page
   */
  public calculateFixPredictions(plan: FlightPlan, fixInfo: WTLineFixInfoData, calcData: WTLineFixInfoCalculatedData): void {
    if (!plan) {
   ...
   */
  private clearCalculatedFixData(calcData: WTLineFixInfoCalculatedData, skipLatLon = false): void {
   ...
   */
  private predictIntersection(plan: FlightPlan, fixInfo: WTLineFixInfoData, intersection: FlightPlanIntersection, predictions: BearingDistPredictions | LatLonPredictions): void {
   ...
   */
  public calculateIntersectionBearingDistances(fixInfos: WTLineFixInfoData[], calcDatas: WTLineFixInfoCalculatedData[]): void {
   ...
   */
  public clearFixIntersections(fixInfo: WTLineFixInfoData): void {
   ...
   * @param fixInfos All of the fix infos
   */
  private clearAllFixIntersections(fixInfos: WTLineFixInfoData[]): void {
   ...
   * @param fixInfos All of the fix infos
   */
  public calculateAllIntersections(plan: FlightPlan, fixInfos: WTLineFixInfoData[]): void {
   ...
   */
  private calculateIntersectionsForLeg(leg: LegDefinition, globalLegIndex: number, isActiveLeg: boolean, fixInfos: WTLineFixInfoData[]): boolean {
    ...
    for (const vector of leg.calculated.flightPath) {
      ...
      const vectorCircle = FlightPathUtils.setGeoCircleFromVector(vector, WTLineFixInfoCalculator.geoCircleCache[0]);
      const vectorStart = WTLineFixInfoCalculator.geoPointCache[0].set(vector.startLat, vector.startLon);
      const vectorEnd = WTLineFixInfoCalculator.geoPointCache[1].set(vector.endLat, vector.endLon);
   ...
   */
  private calculateAbeamCircle(fixInfo: WTLineFixInfoData, pathCircle: GeoCircle): GeoCircle {
    return WTLineFixInfoCalculator.geoCircleCache[1].setAsGreatCircle(fixInfo.fixLocationCartesian, pathCircle.center);
  }
   ...
   * @returns A great circle
   */
  private calculateFixBearingCircle(fixInfo: WTLineFixInfoData, bearing: number): GeoCircle {
    return WTLineFixInfoCalculator.geoCircleCache[1].setAsGreatCircle(fixInfo.fixLocationCartesian, bearing);
  }
   ...
   */
  private calculateFixDistanceCircle(fixInfo: WTLineFixInfoData, distance: number): GeoCircle {
    return WTLineFixInfoCalculator.geoCircleCache[1].set(fixInfo.fixLocationCartesian, UnitType.GA_RADIAN.convertFrom(distance, UnitType.METER));
  }
  ...
  private calculateLatitudeCrossingCircle(latitude: number): GeoCircle {
    ...
    const latitudePoint = WTLineFixInfoCalculator.geoPointCache[2];
    if (latitude < 0) {
      // south pole
      WTLineFixInfoCalculator.geoPointCache[2].set(-90, 0);
    } else {
      // north pole
      WTLineFixInfoCalculator.geoPointCache[2].set(90, 0);
    }
    return WTLineFixInfoCalculator.geoCircleCache[1].set(latitudePoint, (1 - Math.abs(latitude / 90)) * Math.PI / 2);
  }
  ...
  private calculateLongitudeCrossingCircle(longitude: number): GeoCircle {
    ...
    const longitudePoint = WTLineFixInfoCalculator.geoPointCache[2].set(0, longitude);
    return WTLineFixInfoCalculator.geoCircleCache[1].setAsGreatCircle(longitudePoint, 0);
  }
  ...
  private calculateFlightPathIntersection(
    ...
    const numIntersections = intersectionCircle.intersection(vectorCircle, WTLineFixInfoCalculator.vec3Cache);
    if (numIntersections > 0) {
      for (let i = 0; i < numIntersections; i++) {
        const intersection = WTLineFixInfoCalculator.vec3Cache[i];
