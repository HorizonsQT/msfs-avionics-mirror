...
import { WTLineFixInfoCalculator } from './WTLineFixInfoCalculator';
import { WTLineFixInfoCalculatedData, WTLineFixInfoData, WTLineFixInfoFlightPlanData, WTLineFixInfoMarker, WTLineFixInfoWaypoint } from './WTLineFixInfoData';

/** Events for fix info data */
export interface WTLineFixInfoEvents {
  /** The index of a fix info that was updated */
  fix_info_calculated: number,
}

/** Available options for configuring fix info. */
export interface WTLineFixInfoOptions {
...
export class WTLineFixInfoManager {
  private needPredictionsUpdate = false;
  private needIntersectionsUpdate = false;
  private ignoreSync = false;

  private readonly syncDataCache: WTLineFixInfoFlightPlanData[];

  private readonly fixInfos: WTLineFixInfoData[];

  /** FIX INFO page data, {@link WTLineFixInfoOptions.numberOfFixes} elements */
  private readonly calculatedData;

  public readonly ndMarkers = ArraySubject.create<WTLineFixInfoMarker>();
  private readonly ndMarkersCache: WTLineFixInfoMarker[];
  public readonly ndWaypoints = ArraySubject.create<WTLineFixInfoWaypoint>();
  private readonly ndWaypointsCache: WTLineFixInfoWaypoint[] = [];

  private readonly calculator: WTLineFixInfoCalculator;
  ...
  private readonly options: Required<WTLineFixInfoOptions>;
  ...
  constructor(
    ...
    options?: Partial<WTLineFixInfoOptions>,
    ...
    this.calculatedData = Array.from({ length: this.options.numberOfFixes }, (): WTLineFixInfoCalculatedData => ({
      fixIdent: null,
      fixBearing: null,
      fixDistance: null,
    ...
    this.calculator = new WTLineFixInfoCalculator(this.bus, this.options.maxDistance, this.activeRoutePredictor);
  ...
  private onPlanUserDataSet(event: FlightPlanUserDataEvent): void {
    if (event.planIndex === this.activeRoutePlanIndex && event.key === WTLineFmsUtils.USER_DATA_KEY_FIX_INFO) {
  ...
  private onUpdate(): void {
    ...
    for (let i = 0; i < this.options.numberOfFixes; i++) {
      ...
      this.bus.getPublisher<WTLineFixInfoEvents>().pub('fix_info_calculated', i, false, false);
    }
   ...
   */
  private gatherNdMarkers(): WTLineFixInfoMarker[] {
    for (const [i, fixInfo] of this.fixInfos.entries()) {
  ...
  /**
   * Check if two {@link WTLineFixInfoMarker} instances are equal
   * @param a first instance
   * @param b second instance
   * @returns true if the two instances are equal
   */
  private ndMarkerEquality(a: WTLineFixInfoMarker, b: WTLineFixInfoMarker): boolean {
   ...
   */
  private gatherNdWaypoints(): WTLineFixInfoWaypoint[] {
  ...
  /**
   * Check if two {@link WTLineFixInfoWaypoint} instances are equal
   * @param a first instance
   * @param b second instance
   * @returns true if the two instances are equal
   */
  private ndWaypointEquality(a: WTLineFixInfoWaypoint, b: WTLineFixInfoWaypoint): boolean {
   ...
   */
  public getCalculatedData(fixIndex: number): WTLineFixInfoCalculatedData | undefined {
   ...
   */
  public getEnteredData(fixIndex: number): WTLineFixInfoData | undefined {
   ...
   */
  private getFlightPlanFixInfos(planIndex: number): WTLineFixInfoFlightPlanData[] | undefined {
    const plan = this.flightPlanner.getFlightPlan(planIndex);

    return plan.getUserData<WTLineFixInfoFlightPlanData[]>(WTLineFmsUtils.USER_DATA_KEY_FIX_INFO);
  }
   ...
   */
  private setFlightPlanFixInfos(planIndex: number, fixInfos: readonly WTLineFixInfoFlightPlanData[]): void {
    const plan = this.flightPlanner.getFlightPlan(planIndex);

    plan.setUserData(WTLineFmsUtils.USER_DATA_KEY_FIX_INFO, fixInfos);
  }
