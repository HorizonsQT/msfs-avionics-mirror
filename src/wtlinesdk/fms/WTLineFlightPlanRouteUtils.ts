...
import { WT21LegDefinitionFlags } from './WTLineFmsUtils';

/**
 * Methods for working with WT21 flight plan routes.
 */
export class WTLineFlightPlanRouteUtils {
    ...
    for (const segment of flightPlan.segmentsOfType(FlightPlanSegmentType.Enroute)) {
      ...
      const enrouteLegs = await Promise.all(
        segment.legs
          ...
          .map(leg => WTLineFlightPlanRouteUtils.legToFlightPlanRouteEnrouteLeg(facLoader, leg.leg))
      );
