 ...
 */
export class WTLineAlternatePredictor {
  /**
  ...
  public async update(): Promise<void> {
    if (this.planner.hasFlightPlan(WTLineFmsUtils.PRIMARY_ACT_PLAN_INDEX)) {
      const plan = this.planner.getFlightPlan(WTLineFmsUtils.PRIMARY_ACT_PLAN_INDEX);
      ...
      if (destinationFacilityIcao && destinationFacilityIcao !== ICAO.emptyIcao) {
        ...
        if (destinationFacility) {
          ...
          if (destinationPredictions) {
            ...
            const altnFacilityIcao = plan.getUserData(WTLineFmsUtils.USER_DATA_KEY_ALTN) as string;
