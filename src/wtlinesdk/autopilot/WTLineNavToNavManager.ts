...
import { WTLineFmsUtils } from '../fms';
import { WTLineLegacyMessageIDs } from '../message';
import { WTLineMessageService } from '../message/WTLineMessageService';
import { WTLineGhostNeedleControlEvents } from '../navigation';
import { WTLineControlEvents } from '../publishers';
import { CDIScaleLabel, WTLineLNavDataEvents } from './WTLineLNavDataEvents';

/** NavToNav manager using the state pattern */
export class WTLineNavToNavManager implements NavToNavManager2 {
  /** @inheritDoc */
  ...
  public constructor(
    ...
    public readonly messageService: WTLineMessageService<WTLineLegacyMessageIDs>,
  ) {
    ...
    this.bus.getSubscriber<WTLineControlEvents>().on('approach_details_set').handle(x => {
      if (WTLineFmsUtils.isLocalizerApproach(x.approachType)) {
        this.approachFrequency.set(x.referenceFacility?.freqMHz ?? null);
      } else {
        this.approachFrequency.set(null);
      }
    });
    this.bus.getSubscriber<WTLineLNavDataEvents>().on('lnavdata_cdi_scale_label').whenChanged().handle(x => {
      this.scalingLabel = x;
...
abstract class NavToNavState {
  // eslint-disable-next-line jsdoc/require-jsdoc
  public constructor(
    protected readonly navToNav: WTLineNavToNavManager,
...
class NavToNavTunePending extends NavToNavState {
  ...
  public enter(): void {
    ...
    if (isAutoTuning) {
      ...
      this.navToNav.messageService.post(WTLineLegacyMessageIDs.LOC_WILL_BE_TUNED);
  ...
  public exit(): void {
    ...
    this.navToNav.messageService.clear(WTLineLegacyMessageIDs.LOC_WILL_BE_TUNED);
...
class NavToNavArmed extends NavToNavState {
  ...
  private setGhostIndicatorsArmed(isArmed: boolean): void {
    this.bus.getPublisher<WTLineGhostNeedleControlEvents>()
