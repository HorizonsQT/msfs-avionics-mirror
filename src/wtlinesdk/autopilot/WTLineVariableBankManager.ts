...
export enum WTLineMaxBankIndex {
  FULL_BANK,
  HALF_BANK
}
 ...
 */
export class WTLineVariableBankManager {
  ...
  private readonly maxBankIndex: ConsumerSubject<WTLineMaxBankIndex>;
  ...
  constructor(private readonly bus: EventBus, private readonly apValues: APValues) {
    ...
    this.maxBankIndex = ConsumerSubject.create(ap.on('ap_max_bank_id').whenChanged(), WTLineMaxBankIndex.FULL_BANK);
   ...
   */
  private handleMaxBankIndexChanged = (index: WTLineMaxBankIndex): void => {
    const options: Partial<FlightPathCalculatorOptions> =
      { bankAngle: index === WTLineMaxBankIndex.FULL_BANK ? this.fullBankCalculatorValue : this.halfBankCalculatorValue };
  ...
  private handleAltitudeUpdated = (altitude: number): void => {
    ...
    if (altitude > this.halfBankActivationAltitude && this.maxBankIndex.get() === WTLineMaxBankIndex.FULL_BANK && !this.halfBankSetAtActivationAltitide) {
      this.halfBankSetAtActivationAltitide = true;
      SimVar.SetSimVarValue('K:AP_MAX_BANK_SET', 'number', WTLineMaxBankIndex.HALF_BANK);
    } else if (altitude < this.halfBankActivationAltitude && this.maxBankIndex.get() === WTLineMaxBankIndex.HALF_BANK && !this.fullBankSetAtActivationAltitide) {
      this.fullBankSetAtActivationAltitide = true;
      SimVar.SetSimVarValue('K:AP_MAX_BANK_SET', 'number', WTLineMaxBankIndex.FULL_BANK);
    }
  ...
  private handleLateralActiveChanged = (mode: APLateralModes): void => {
    ...
    if (mode === APLateralModes.GPSS) {
      ...
      if (altitude > this.halfBankActivationAltitude && this.maxBankIndex.get() === WTLineMaxBankIndex.FULL_BANK) {
        SimVar.SetSimVarValue('K:AP_MAX_BANK_SET', 'number', WTLineMaxBankIndex.HALF_BANK);
      } else if (altitude < this.halfBankActivationAltitude && this.maxBankIndex.get() === WTLineMaxBankIndex.HALF_BANK) {
        SimVar.SetSimVarValue('K:AP_MAX_BANK_SET', 'number', WTLineMaxBankIndex.FULL_BANK);
      }
    }
