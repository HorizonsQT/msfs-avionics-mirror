...
/**
 * A utility class for working with WT Line autopilots.
 */
export class WTLineAPUtils {
  /**
  ...
  public static navInterceptCurve(distanceToSource: number, deflection: number, xtk: number, tas: number, isLoc?: boolean): number {
    if (isLoc) {
      return WTLineAPUtils.localizerInterceptCurve(deflection, xtk, tas);
    } else {
      return WTLineAPUtils.defaultInterceptCurve(xtk, tas);
    }
  }
  ...
  public static localizerInterceptCurve(deflection: number, xtk: number, tas: number): number {
    ...
    const interceptAngle = WTLineAPUtils.calculateTurnBasedInterceptAngle(turnRadiusMeters, xtkMeters);
